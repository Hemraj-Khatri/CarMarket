        ...
        <>
                  ...
                  Object.entries(getCarById?.data?.features)
                    ...
                    .map(([feature, _], index) => (
                      <li key={index} className='flex items-center'>
                        <SiTicktick className='text-blue-600 mx-2' />{feature}
                      </li>
                    ))}
              </ul>
            </div>
          ...
          <div className='w-full'>
              ...
              <ul>
                {getCarById?.data &&
                  Object.keys(getCarById?.data)
                    .filter(key => ![
                      "description", "images", "createdAt", "updatedAt", "__v",
                      "originalPrice", "sellingPrice", "features", "_id"
                    ].includes(key))
                    .map((key, index) => (
                      <div key={index}>
                        <li className="flex items-center gap-2 py-3">
                          ...
                          {key} <span className='ms-auto pe-5'>{getCarById?.data[key]?.toString()}</span>
                        </li>
                        <hr />
                      </div>
                    ))}
            ...
            <div className='shadow-lg hover:shadow-2xl flex flex-col items-center py-5 max-w-md'>
              ...
              <img src={getCarById?.data?.createdBy?.profilePhoto} alt="Owner" width="40%" />
